...
def richer_than_england
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
      gdp/population > (SELECT gdp/population FROM countries WHERE name = 'United Kingdom') AND continent = 'Europe';
  SQL
end
...
def neighbors_of_certain_b_countries
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
      continent = (SELECT continent FROM countries WHERE name = 'Belize') OR
...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
      continent = (SELECT continent FROM countries WHERE population = ALL());
  SQL
end
