...
def xnor_select(arr, prc1, prc2)
    newArr = []
    arr.each {|ele| newArr << ele if (prc1.call(ele) && prc2.call(ele)) || (!prc1.call(ele) && !prc2.call(ele))}
    newArr
end
...
def multi_map(arr, num = 1, &prc)
    newArr = arr.map do |ele|
        newEle = ele 
        num.times {newEle = prc.call(newEle)}
        newEle
    end
    newArr
end
...
def proctition(arr, &prc)
    newArr = arr.select { |e| prc.call(e) }
    arr.each { |ele| newArr << ele if !prc.call(ele) }
    newArr
end
...
def chain_map(val, prc_arr)
    prc_arr.inject(val) { |acc, ele| ele.call(acc) }
end
...
def procipher(sent, hash)
    ...
    words.each do |word|
        newWord = word
        hash.each do |k,v|
            newWord = v.call(newWord) if k.call(word)
        end
        newArr << newWord
    end
    newArr.join(" ")
